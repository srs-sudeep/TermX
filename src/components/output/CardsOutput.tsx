...
export function CardsOutput({ cards }: CardsOutputProps) {
  return (
    <div className="space-y-4">
      {cards.map((card, idx) => (
        <div key={idx} className="border-l-2 border-[var(--border)] pl-4 space-y-1">
          {}
          <p className="text-[var(--accent)] font-bold">{card.title}</p>

          {}
          {card.subtitle && <p className="text-[var(--muted)] text-[0.9em]">{card.subtitle}</p>}

          {}
          {card.body && <p className="text-[var(--fg)] whitespace-pre-wrap">{card.body}</p>}
            ...
            <ul className="list-none space-y-0.5 mt-1">
              {card.bullets.map((b, bIdx) => (
                <li key={bIdx} className="flex items-start gap-2 text-[var(--fg)]">
                  <span className="text-[var(--muted)] select-none flex-shrink-0 mt-px">•</span>
                  <span>{b}</span>
                </li>
              ))}
            </ul>
